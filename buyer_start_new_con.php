<!--start new conversation page for buyers-->
			...
			if(isset($_GET['user']) && !empty($_GET['user'])){
				...
				<form method='post' >
					...
					if(isset($_POST['message']) && !empty($_POST['message'])) {
						...
						if( $result["count(*)"] >= 1){
							echo "<p><b>Conversation already started!</b></p>";
							...
							header('location: buyer_conversation.php?hash='.$random_number);
				...
				while($run_user = mysqli_fetch_array($user_list)){
					...
					echo "<p><a href='buyer_start_new_con.php?user=$user'>$username</a></p>";
				}
			}
			?>
