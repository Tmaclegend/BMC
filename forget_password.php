...
#echo 'entered btn';
	...
	$database_array = mysqli_query($con,"SELECT * FROM users WHERE username = '$input_name'");
	$database_array = mysqli_fetch_assoc($database_array);
	$username = $database_array['username'];
	if(!empty($username)){
		$email =  $database_array['email'];
		if(strcmp($input_email, $email) != 0){
			$mess = "E-mail not match!";
			echo "<script type='text/javascript'>alert('$mess');</script>";
		}
		else{
			$password = $database_array['password'];
			$mess = "BE CAREFUL! Your password will be shown here.";
			echo "<script type='text/javascript'>alert('$mess');</script>";
			$mess = "Your password is: ".$password;
			echo "<script type='text/javascript'>alert('$mess');</script>";
		}
	}
	else{
...
<div class="banner-top">
	<div class="container">
		...
		<em></em>
		<h2><a href="index.php">Home</a><label>/</label>Forget Password</h2>
	</div>
</div>
<!--login-->
<div class="container">
		<div class="login">
			...
			<?php if ($reg_success == 0) echo "<h4 style=\"color:#FF6C6C;\"c>This email is already registered</h4><br>"; else if ($reg_success == 2) echo 
			"<h4 style=\"color:#FF6C6C;\"c>Registered Successfully!</h4><br>" ?>
			<div class="login-mail">
				...
				<label class="hvr-skew-backward">
					<input type="submit" name="register_btn" value="Check Password">
				</label>
